...
const slides = [
  ...
  { id:'back-office', type:'back-office', title:'The platforms behind the customer channels', lede:'Customer journeys are supported by the current Admin Portal and the first focused products being separated from it.' },
  ...
  { id:'strategy-meaning', type:'strategy-meaning', title:'The strategy changes how Manafa owns and evolves products', lede:'The 2026 direction is not only a system redesign. It moves us from delivering features inside one broad Admin Portal to managing a connected suite of products with clear audiences, owners and outcomes.' },
...
function RenderSlide({ slide, index, total, onSources }) {
  ...
  return <section className={`slide content content--${slide.type}${dark?' dark':''}`}>
    ...
    {slide.type === 'customer-channels' && <div className="customer-channels">
      <article className="channel-panel channel-panel--borrower"><div className="channel-copy"><img src={`${A5}manafa-business-icon.jpg`} alt="Manafa Business app icon"/><span>WEB · MOBILE</span><h2>Borrower relationship</h2><p>Businesses discover financing, apply and track requests through Manafa Business.</p></div><div className="channel-phones"><figure><img src={`${A5}manafa-business-02.png`} alt="Manafa Business home journey"/></figure><figure><img src={`${A5}manafa-business-01.png`} alt="Manafa Business financing request journey"/></figure></div></article>
      <article className="channel-panel channel-panel--investor"><div className="channel-phones"><figure><img src={`${A6}manafa-investor-01.png`} alt="Manafa investor opportunities journey"/></figure><figure><img src={`${A6}manafa-investor-03.png`} alt="Manafa investor preferences journey"/></figure></div><div className="channel-copy"><img src={`${A6}manafa-investor-icon.jpg`} alt="Manafa investor app icon"/><span>WEB · MOBILE</span><h2>Investor relationship</h2><p>Investors discover opportunities, invest and monitor their portfolios through Manafa.</p></div></article>
    </div>}

    {slide.type === 'back-office' && <div className="back-office"><article className="admin-core"><span>CURRENT FOUNDATION</span><div className="admin-orbit"><strong>ADMIN</strong><small>One broad operational portal</small></div><p>The current Admin Portal still brings many operational domains together in one place.</p></article><div className="back-office-shift"><ArrowRight/><span>FIRST SPIN-OFFS</span></div><div className="spin-offs">{[
      ['CRM','Customer relationships, activities and servicing context.',Users,'live'],
      ['Invoice Management','Invoice processing, controls and financial operations.',ReceiptText,'build'],
      ['SCF FinOps','Buyer-led SCF operations, funding and settlement workflows.',Waypoints,'live'],
    ].map(([name,copy,Icon,status])=><article key={name}><IconBubble Icon={Icon} tone={status==='live'?'teal':'violet'}/><span>{status==='live'?'LIVE':'IN DEVELOPMENT'}</span><h2>{name}</h2><p>{copy}</p></article>)}</div></div>}
    ...
    {slide.type === 'strategy-meaning' && <div className="strategy-meaning"><div className="strategy-shift"><article className="strategy-now"><span>CURRENT MODEL</span><div className="strategy-admin"><strong>ADMIN</strong><small>One broad internal portal</small></div><p>Features accumulate inside one shared system and ownership follows incoming demand.</p></article><div className="strategy-arrow"><ArrowRight/><span>2026<br/>SHIFT</span></div><article className="strategy-target"><span>TARGET MODEL</span><div className="target-layers"><b>Audience-facing channels</b><b>Focused domain products</b><b>Shared platform capabilities</b></div><p>Products have clear users, accountable owners, roadmaps and measures.</p></article></div><div className="strategy-outcomes">{[
      ['01','Clear ownership','One accountable product owner and outcome for every domain.'],
      ['02','Modular delivery','Focused products can evolve without expanding risk across the entire suite.'],
      ['03','Customer proximity','Customer logic stays in channels; operational logic stays in domain products.'],
      ['04','Measurable value','Adoption, quality, cycle time and business impact guide the roadmap.'],
    ].map(([n,t,c])=><article key={n}><b>{n}</b><div><strong>{t}</strong><p>{c}</p></div></article>)}</div></div>}
