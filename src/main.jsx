...
const slides = [
  ...
  { id:'back-office', type:'back-office', title:'The platforms behind our customer channels', lede:'Our customer channels connect to an internal platform landscape made up of the Admin Portal and three additional back-office platforms.' },
...
function RenderSlide({ slide, index, total, onSources }) {
  ...
  return <section className={`slide content content--${slide.type}${dark?' dark':''}`}>
    ...
    {slide.type === 'back-office' && <div className="back-office platform-landscape"><article className="admin-platform"><div className="platform-heading"><IconBubble Icon={Network}/><div><span>CORE OPERATIONS PLATFORM</span><h2>Admin Portal</h2></div></div><p>The broad internal workspace used by Manafa teams to manage financing activity across its operational lifecycle.</p><div className="admin-capabilities">{['Companies & users','Financing requests','Credit & approvals','Opportunity publishing','Servicing & operations'].map(x=><span key={x}>{x}</span>)}</div></article><div className="focused-platforms">{[
      ['CRM','Customer relationship platform','Manages customer profiles, activities, interactions and servicing context.',Users,'blue'],
      ['SCF FinOps','Supply-chain finance operations','Supports buyer-led SCF operations, funding activity and settlement workflows.',Waypoints,'teal'],
      ['Invoice Management','Invoice operations platform','Manages invoice processing, validation, controls and related financial operations.',ReceiptText,'violet'],
