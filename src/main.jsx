...
const slides = [
  ...
  { id:'digital-business', type:'digital-business', title:'What Digital Business leads across the product suite', lede:'Digital Business leads specific channels and products within the wider suite; ownership is assigned product by product—not at the hub level.' },
...
function RenderSlide({ slide, index, total, onSources }) {
  ...
  return <section className={`slide content content--${slide.type}${dark?' dark':''}`}>
    ...
    {slide.type === 'digital-business' && <div className="digital-business ownership-portfolio"><section className="owned-channels"><header><b>01</b><div><span>DIRECT CHANNEL LEADERSHIP</span><h2>Business-facing channels</h2></div></header><div>{['Borrower','Funder','Buyer','Embedded'].map(x=><strong key={x}>{x}</strong>)}</div></section><section className="owned-products"><header><b>02</b><div><span>PRODUCT-BY-PRODUCT LEADERSHIP</span><h2>Internal products led by Digital Business</h2></div></header><div className="ownership-domains">{[
      ['Customer domain',['CRM','Client Lifecycle¹','Transaction Management¹']],
      ['Lending domain',['Loan Origination','Loan Management & Collections']],
      ['Counterparty domain',['Funder Management','Buyer Management','SCF FinOps']],
      ['Financial domain',['Ledger','Treasury','Invoice Management & Tax¹']],
      ['Embedded & shared',['Developer Platform','ERP Middleware','Identity & Access¹','Log Management','Knowledge Management']],
    ].map(([domain,items])=><article key={domain}><h3>{domain}</h3>{items.map(x=><span key={x}>{x}</span>)}</article>)}</div></section><footer><span>¹ Raghad kept informed</span><strong>Raghad leads the Investor Channel, Customer Service, Engagement Management and Digital Experience. API Platform remains Development Team-led.</strong></footer></div>}
